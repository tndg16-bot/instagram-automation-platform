...
export default function DMPage() {
  ...
  const [newCampaign, setNewCampaign] = useState({
    ...
    recipients: [] as any[],
    campaign_type: 'simple' as 'simple' | 'sequence',
  });
  ...
  const handleCreateCampaign = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        if (data.campaign.campaign_type === 'sequence') {
          window.location.href = `/workflow?campaign_id=${data.campaign.id}`;
        } else {
  ...
  const handleSendCampaign = async (campaignId: string) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`http://localhost:8000/api/dm/campaigns/${campaignId}/send`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
  ...
  const handleDeleteCampaign = async (campaignId: string) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`http://localhost:8000/api/dm/campaigns/${campaignId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center">
                              ...
                              {campaign.campaign_type === 'sequence' && (
                                <span className="ml-2 inline-flex items-center px-2 py-1 bg-indigo-100 text-indigo-600 rounded-full text-xs font-medium">
                                  🔄
                                </span>
                              )}
                            </div>
                            <div className="mt-2 sm:flex sm:justify-between">
                              <div className="sm:flex">
                                ...
                                <p className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0 sm:ml-6">
                                  <span className="mr-1.5">✅</span>
                                  {campaign.delivered_count || 0} delivered
                                </p>
                              </div>
                            </div>
                          </div>
